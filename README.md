...
- микро-сервер на базе смартфона сразу имеет слот для симки (рассылка уведомлений и получение команд через смс + мобильный интернет), wi-fi и собственный сенсорный дисплей. У подобного сервера есть масса и других преимуществ: низкая цена устройства, встроенный аккумулятор, Bluetooth, микрофон, мобильность и т.д.
- сопряжённое управляющее оборудование используется производителя [ab-log.ru](https://ab-log.ru/)
- ~~хранение настроек (и прочих данных) в облаке. облачное хранилище "поверх почтовых протоколов" (SMTP/POP3/IMAP). Таким образом хранение данных будет бесплатным и доступным любому пользователю. Достаточно иметь почтовый ящик.~~
- удалённый доступ к управлению портами, настройкам и рассылка уведомлений через Intenet/TelegramBot/SMS. "Удалённые пользователи" в зависимости от настроек имеют доступ непосредственно к выходным портами (вкл/выкл оборудования) и/или получение уведомлений от входных портов (датчики) через SMS/Telegram/E-mail. Так же доступен механизм выполнения сценариев (пакетных заданий), с элементами расширенной логики (проверка условия перед выполнением команды и пр.).
- Мультиязычность. Локализация организована [стандартными средствами Xamarin.Android](https://github.com/badhitman/ab-log-app/blob/master/server%20configurator/Resources/values/strings.xml). Таким образом добавление нового языка предельна проста.
...
Одна из особенностей такой оптимизации заключается в том, что приложение, работающее более трёх дней будет остановлено операционной системой.
Что бы избежать такого порведения - следует настроить вашу OS, отключив отпитимизацию для нашего приложения.
Если смартфон станет выполнять роль сервера, то разумеется, для него следует предусмотреть постоянное подключение к питанию (к зарадному устройству).
Без зарядного устройства смартфон сможет какое-то время работать, но срок автономной работы исчисляется сутками/часами, а системы "умный дом" должны рботать постоянно.
...
А это демонстрация расположения настроек в BQ:

![screenshot-19](./screenshots/screenshot-19.jpg)
...
В приложении доступны два меню. Основное вызывется кнопкой сверху-слева и вспомагательное кнопкой сверху-справа.
В основном меню расположены пункты:
 - **MegaD-2516** - доступ к устройствам. Создание, редктирование и т.п.
 - **Пользователи** - управление пользователями (удалённого управления/оповещения)
 - **Сценарии** - управление сценариями (пакетными заданиями).
 - **Службы** - настройка/зпуск/остановка служб HTTP/MQTT/TelegramBot
 - и две ссылки: на сайт (в т.ч. магазин и форум) производителя контроллеров, а так же на страницу исходников данной программы (т.е. сюда)

 Вспомагательное меню располагает в себе:
  - **О программе** - общая информация о приложении. Сборка **debug** дополнительно выводит палитру системных цветов (используется мною для подбора цветов элементов управления)
  - **Telegram Bot** - настройка подсистемы TelegramBot в т.ч. связь пользователей Telegram с пользователями приложения.
  - **Логи** - просмотр логов. Примечание: логи можно просматривать в т.ч. через TelegramBot (если он настроен/используется).
...
При изменении в системных настройках - эти изменения втоматически сохрнются в настройках устройства в БД приложения.
...
Однако в системных настройках устройства существует одно важное отличие от родного: Наименование портов. 

В оригинале устройство не имеет возможности задавать имена портам.
В данном же случае такое поле добавлено, но хранится это имя в БД приложения, а не в памяти самого устройства.
...
В приложении можно сформировать пакетные задания. Пакетные задания состоят из серии команд, запуск которых возможен как удалённо так и по событию: сработал порт.
Кроме того каждой команде в пакете можно установить паузу ожидания и/или условие проверки перед выполнением.
Команда может быть быть либо на установку состояния конкретному порту, либо отправка низкоуровневой команды контроллеру.
Дополнительно комнда может указать на необходимость прекратить выполнение скрипта либо на переход в другой произвольный скрипт на произвольную позицию в этом скрипте для продолжения выполнения уже внутри нового пакетного контекста/задания.
Таким образом появляется возможность рекурссионного обхода блока команд до выполнения требуемого условия или ветвления логики в зависимости от состояния портов.
...
> регистрация и настройка Telegram бота проста, поэтому данная процедура не описывается здесь.
Напомню имя официального и бота для регистрации личных ботов: @BotFather. При желании можно сразу прописать несколько команд:
 - /hardwares - устройства
 - /scripts - сценарии
 - /logs_at_0 - логи

 Впрочем это не обязательно. Достаточно отправить боту любой текст (не являющийся встроеной командой) и бот ответит вам перечнем доступных команд.
 Примечание: бот отвечает только тем клиентам, которые уже свзязаны с пользователями системы, а "незнакомцам" бот ни чего не ответит.
 В то же время, бот регистрирует в БД всех писавших ему клиентов без исключения. В окне настроек бота можно ознакомиться со всеми клиентами и сразу видеть каким пользователям они назнчены.
 Собственно для того что бы связать клиента с пользователем нужно написать любое сообщение боту с клиента Telegram (по началу бот ни чего не ответит), после чего это пользователь появится в разделе "Telegram bot".
 Только после этого требуемого клиента связывают с нужным пользователем - чем вы назначаете и права клиенту (получение уведомлений и/или удлённое управление).
...
> просмотр логов доступен и через Telegram. Сообщения получаются порциями по 10 начиная с конца (с самого свежего сообщения) в сторону начала (к самому старому).
