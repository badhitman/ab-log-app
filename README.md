...
На данный момент MQTT в приложении функцианирует, но его функционал ограничен. Т.е. сам брокер функционирует исправно, но ни как не регирует на сообщения от устройств.
Брокер позволяет подписаться н топики и отправлять/получать сообщения, но всё это в холостую.
