...
Напомню имя официального и бота по регистрации личных ботов: @BotFather. Срзу после регистрации при желании можно сразу прописать несколько команд:
 ...
 Примечание: бот отвечает только тем клиентам, которые уже свзязаны с пользователями системы. В то же время бот регистрирует в БД всех писавших ему клиентов когда либо без исключения.
 Собственно для того что бы изначально связать клиента с пользователем нужно написать любое сообщение боту с клиента Telegram (по началу бот ни чего не ответит), после чего это пользователь появится в разделе "Telegram bot".
